(function () {
  ...
  function detail() {
    ...
    const rows = [['Change since prior snapshot', `${candidate.change.status}${candidate.change.scoreDelta === null ? '' : ` (${candidate.change.scoreDelta > 0 ? '+' : ''}${candidate.change.scoreDelta} score)`}`], ['Explainable score', `${candidate.score.total}/100 · ${candidate.score.coverage} coverage`], ['Latest observed buy', `${candidate.token} · ${dollars(candidate.observedAmountUsd)} · ${time(candidate.observedAt * 1000)}`], ['Realized profit · 7D', dollars(evidence.realizedProfit)], ['Realized profit · 30D', dollars(evidence.realizedProfit30d)], ['Win rate · 7D / 30D', `${pct(evidence.winRate)} / ${pct(evidence.winRate30d)}`], ['Realized PnL ratio', pct(evidence.pnlRatio)], ['Buy + sell count', evidence.tradeCount === null ? 'Not returned' : `${evidence.tradeCount} transactions`], ['Token diversity', evidence.tokenCount === null ? 'Not returned' : `${evidence.tokenCount} tokens`], ['GMGN tags', candidate.tags.length ? candidate.tags.join(', ') : 'Not returned']];
