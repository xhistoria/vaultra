  ...
  return async function handler(req, res) {
    ...
    try {
      ...
      const stats = addresses.length
        ? unwrap(await (await fetchImpl(providerUrl('/v1/user/wallet_stats', { chain, wallet_address: addresses, period: '7d' }, now, randomUUID), { headers })).json())
        : [];
      const result = buildScanResult({ smartMoney: trades, stats });
      return res.status(200).json({ source: 'GMGN OpenAPI', chain, generatedAt: new Date(now()).toISOString(), candidates: result.candidates });
    } catch {
